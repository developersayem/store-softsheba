 ...
 */
async function issueAccessToken(credentials: PathaoCredentials) {
  try {
    ...
    throw new ApiError(
      400,
      error?.response?.data?.message || "Pathao token generation failed",
    );
  }
}
 ...
 */
function mapToPathaoOrder(order: any, storeId: number) {
...
export const pathaoCourier = {
  /**
   * SINGLE ORDER CREATE
   */
  async send(order: any, credentials: PathaoCredentials) {
    ...
    try {
      ...
      throw new ApiError(
        400,
        error?.response?.data?.message || "Pathao order creation failed",
      );
    }
  },
   ...
   * BULK ORDER CREATE
   */
  async sendMany(orders: any[], credentials: PathaoCredentials) {
    const accessToken = await issueAccessToken(credentials);

    try {
      const payload = {
        orders: orders.map((order) =>
          mapToPathaoOrder(order, Number(credentials.storeId)),
        ),
      };
      // console.log("payload:",payload)

      const response = await axios.post(
        `${BASE_URL}/aladdin/api/v1/orders/bulk`,
        payload,
        {
          headers: {
            "Content-Type": "application/json;",
      ...
      throw new ApiError(
        400,
        error?.response?.data?.message || "Pathao bulk order failed",
      );
    }
  },
  async getStatus(trackingCode: string, credentials: PathaoCredentials) {
    const accessToken = await issueAccessToken(credentials);
    try {
      ...
      throw new ApiError(
        400,
        error?.response?.data?.message || "Pathao get status failed",
      );
    }
  },
};
