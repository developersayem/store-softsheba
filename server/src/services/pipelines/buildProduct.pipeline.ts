...
export function buildProductPipeline(
  query: Record<string, any>,
  onlyPublished = false,
  filterOverrides: Record<string, any> = {},
  variantStatus: "active" | "inactive" | "all" = "active",
) {
  const page = Math.max(1, parseInt(query.page as string) || 1);
  const limit = query.limit !== undefined ? parseInt(query.limit as string) : 100000;
  const skip = (page - 1) * limit;

  const filters: any = { ...filterOverrides };
  if (onlyPublished) filters.is_published = true;
  ...
  if (!filterOverrides._id && !filterOverrides.slug) {
    ...
    if (query.category) {
      const cats = (query.category as string)
        .split(",")
        .map((c) => new Types.ObjectId(c.trim()));
      filters.categories = { $in: cats };
    }
    if (query.collection) {
      const cols = (query.collection as string)
        .split(",")
        .map((c) => new Types.ObjectId(c.trim()));
      filters.collections = { $in: cols };
    }

    if (query.brand) filters.brand = new Types.ObjectId(query.brand);
    ...
    if (query.minPrice || query.maxPrice) {
      ...
      if (query.minPrice) filters.regular_price.$gte = Number(query.minPrice);
      if (query.maxPrice) filters.regular_price.$lte = Number(query.maxPrice);
    }
  }
  ...
  const variantMatchCondition: any = {
