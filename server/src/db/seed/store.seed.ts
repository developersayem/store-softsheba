...
export const storeSettingsRules = async () => {
  ...
  const defaultSettings: any = {
    ...
    footer: {
      ...
      about: {
        ...
        description:
          "ShopXet একটি বিশ্বস্ত অনলাইন প্ল্যাটফর্ম, যেখানে আপনার ব্যবসায়ের প্রয়োজনীয় সকল পণ্য পাওয়া যায়। আমরা মূলত উন্নত মানের ই-কমার্স সমাধান প্রদান করে থাকি। আপনার ব্যবসাকে রাখুন ঝকঝকে ও আধুনিক — ShopXet-এর সাথে।",
    ...
    pages: {
      ...
      contact_page: {
        title: "Contact us",
        description:
          "We're here to help! Whether you have a question about your order, need assistance with a product, or just want to share feedback, our team is ready to assist you.",
        address: "Choto bazar, Mymensingh-2200",
        phone: "+880 1969-888555",
        email: "shoxet.com@gmail.com",
      },
        ...
        faqs: [
          {
            ...
            answer:
              "We offer a range of cleaning products and tools suitable for households and commercial spaces.",
          },
          {
            question: "How do I place an order?",
            answer:
              "Placing an order with us is easy! You can browse our catalog, add items to your cart, and proceed to checkout online.",
          },
          {
            question: "What are your payment options?",
            answer:
              "We offer several payment options for your convenience, including mobile banking, credit/debit card payments, and cash on delivery.",
          },
          {
            question: "What is your return policy?",
            answer:
              "Please review our Refund and Return policy page for detailed information on returns.",
          },
        ],
      },
    },
  ...
  const allSettings = await storeSettings.find({ 
    $or: [
      { key: "global" },
      { key: { $exists: false } },
      { key: null }
  ...
  const settingsByStore = new Map<string, any[]>();
  allSettings.forEach((s) => {
    const sId = s.storeId?.toString() || "legacy";
    if (!settingsByStore.has(sId)) settingsByStore.set(sId, []);
  ...
  for (const [sId, list] of settingsByStore.entries()) {
    if (list.length > 1) {
      console.log(
        `➤ Found ${list.length} duplicates for store [${sId}]. Cleaning up...`,
      );
      const toKeep =
        list.find((s) => s.site_name !== "ShopXet Store") || list[0];
      ...
      // If the preserved one has no key, give it one
      if (toKeep.key !== "global") {
        await storeSettings.updateOne({ _id: toKeep._id }, { $set: { key: "global" } });
      }

      console.log(
        `✔ Redundant settings removed for store [${sId}]. Preserved: ${toKeep.site_name}`,
      );
    }
  }
  ...
  if (!settings) {
    ...
    if (defaultStore) {
      ...
      console.error(
        "❌ Failed to create StoreSettings: No default store found to link!",
      );
      return;
    }
  ...
  for (const key of legalKeys) {
    const existingPages = settings.legal_pages as any;
    const defaultPages = defaultSettings.legal_pages as any;
  ...
  if (Object.keys(update).length > 0) {
    console.log(
      `➤ Synchronizing missing store content: ${Object.keys(update).join(", ")}`,
    );
    await storeSettings.updateOne({ key: "global" }, { $set: update });
