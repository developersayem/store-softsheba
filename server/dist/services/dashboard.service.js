...
exports.getProfitandLossDataByProductAndCategory = exports.profitAndLossOverview = exports.getOverviewData = void 0;
//dasboard.service.ts
...
const getOverviewData = async ({ statuses, timeframe, frequency, }) => {
    ...
    // ==========================
    const summary = await order_model_1.Order.aggregate([
        {
            $addFields: {
    ...
    const trends = await order_model_1.Order.aggregate([
        {
            $addFields: {
                normalizedStatus: { $toLower: "$status" },
            },
    ...
    const locations = await order_model_1.Order.aggregate([
        {
            $addFields: {
                normalizedStatus: { $toLower: "$status" },
            },
    ...
    const items = await order_model_1.Order.aggregate([
        {
            $addFields: {
                normalizedStatus: { $toLower: "$status" },
            },
    ...
    const customers = await order_model_1.Order.aggregate([
        {
            $addFields: {
                normalizedStatus: { $toLower: "$status" },
            },
    ...
    const formattedCustomers = customers.map((c) => ({
        ...
        value: c.value,
        since: (0, dayjs_1.default)(c.firstOrder).format("MMM D, YYYY"),
    }));
    // ==========================
    // WORKFLOW DATA CALCULATIONS
    // ==========================
    // 1. Workflow Status Percentages
    const workflowStatusData = Object.entries(statusMap).map(([status, data]) => ({
    ...
    const cycleTimeAgg = await order_model_1.Order.aggregate([
        { $match: matchStage },
        {
            $group: {
                _id: null,
    ...
    const msToHours = (ms) => ms ? Number((ms / (1000 * 60 * 60)).toFixed(2)) : 0;
    ...
    const cancelledAgg = await order_model_1.Order.aggregate([
    ...
    const totalCanclledCount = cancelledAgg.reduce((sum, item) => sum + item.count, 0);
    const cancelledOrdersData = cancelledAgg.map((item, index) => {
        const percentage = totalCanclledCount > 0
    ...
    const returnedAgg = await order_model_1.Order.aggregate([
        { $addFields: { normalizedStatus: { $toLower: "$status" } } },
    ...
    const totalReturnedCount = returnedAgg.reduce((sum, item) => sum + item.count, 0);
    const returnedOrdersData = returnedAgg.map((item, index) => {
        const percentage = totalReturnedCount > 0
    ...
    const deliveryPartnersAgg = await order_model_1.Order.aggregate([
        { $match: matchStage },
        {
            $group: {
    ...
    // --- PIPELINE 2: Payments ---
    const paymentsAgg = await order_model_1.Order.aggregate([
        { $addFields: { normalizedStatus: { $toLower: "$status" } } },
    ...
    const discountsAgg = await order_model_1.Order.aggregate([
        { $match: matchStage },
        {
            $group: {
                _id: null,
    ...
    return {
        ...
        items: (0, image_resolver_plugin_1.resolveImageUrls)(formattedItems, ["image"]),
        customers: formattedCustomers,
        deliveryPartners: deliveryPartnersAgg.map((dp) => ({
            name: dp._id || "Other",
            count: dp.count,
            value: dp.value,
            percentage: totalCount > 0 ? Number(((dp.count / totalCount) * 100).toFixed(2)) : 0,
        })),
...
const profitAndLossOverview = async (timeframe, statuses) => {
    const dateRange = getDateRange(timeframe);
    const normalizedStatuses = Array.isArray(statuses)
        ? statuses.map((s) => s.toLowerCase())
        : [];
    ...
    const lossAggregation = await order_model_1.Order.aggregate([
    ...
    const profitAggregation = await order_model_1.Order.aggregate([
        {
            $addFields: {
                normalizedStatus: { $toLower: "$status" },
            },
...
exports.profitAndLossOverview = profitAndLossOverview;
const getProfitandLossDataByProductAndCategory = async () => {
    // --- Aggregation for profit by product ---
    const profitByProduct = await order_model_1.Order.aggregate([
        { $addFields: { normalizedStatus: { $toLower: "$status" } } },
        {
            $match: {
                normalizedStatus: {
    ...
    const profitByCategory = await order_model_1.Order.aggregate([
        { $addFields: { normalizedStatus: { $toLower: "$status" } } },
        {
            $match: {
                normalizedStatus: {
...
exports.getProfitandLossDataByProductAndCategory = getProfitandLossDataByProductAndCategory;
