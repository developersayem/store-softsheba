...
async function generateUniqueSlug(name, unit, excludeId) {
    let baseSlug = (0, slugify_1.default)(name, { lower: true, strict: true });
    if (unit) {
        const unitSlug = (0, slugify_1.default)(unit, { lower: true, strict: true });
        baseSlug = `${baseSlug}-${unitSlug}`;
    }
    if (!baseSlug)
        baseSlug = `attr-${Date.now()}`;
    let slug = baseSlug;
    let exists = await attribute_model_1.Attribute.findOne({ slug, _id: { $ne: excludeId } });
    while (exists) {
        slug = `${baseSlug}-${Math.floor(Math.random() * 0xffff).toString(16)}`;
        exists = await attribute_model_1.Attribute.findOne({ slug, _id: { $ne: excludeId } });
    }
    return slug;
}
exports.attributeService = {
    async create(payload) {
        if (!payload.name)
            throw new ApiError_1.ApiError(400, "Attribute name is required");
        // Pass unit to slug generator
        const slug = await generateUniqueSlug(payload.name, payload.unit);
        const attr = await attribute_model_1.Attribute.create({
            name: payload.name,
            slug,
            unit: payload.unit,
    ...
    async update(id, payload) {
        if (!mongoose_1.Types.ObjectId.isValid(id))
            throw new ApiError_1.ApiError(400, "Invalid attribute id");
        const attr = await attribute_model_1.Attribute.findById(id);
        if (!attr)
            throw new ApiError_1.ApiError(404, "Attribute not found");
        // Regenerate slug if Name OR Unit changes
        if ((payload.name && payload.name !== attr.name) ||
    ...
    async delete(id) {
        if (!mongoose_1.Types.ObjectId.isValid(id))
            throw new ApiError_1.ApiError(400, "Invalid attribute id");
        const deleted = await attribute_model_1.Attribute.findByIdAndDelete(id);
        if (!deleted)
            throw new ApiError_1.ApiError(404, "Attribute not found");
        return deleted;
    },
    async deleteMultiple(ids) {
        ...
        const result = await attribute_model_1.Attribute.deleteMany({ _id: { $in: validIds } });
    ...
    async getAll(query) {
        ...
        return attribute_model_1.Attribute.find(filter).sort({ createdAt: -1 });
    },
    ...
    async toggleMultiple(ids, action) {
        ...
        await attribute_model_1.Attribute.updateMany({ _id: { $in: ids } }, { $set: { isActive } });
        return isActive;
    },
    async importAttributes(filePath, ext) {
        const fs = require("fs");
        ...
        try {
            if (ext === "json") {
                ...
                const data = await Promise.all(parsed.map(async (attr) => ({
                    name: attr.name,
                    unit: attr.unit,
                ...
                await attribute_model_1.Attribute.insertMany(data);
            ...
            if (ext === "csv") {
                ...
                for (const row of results) {
                    ...
                    await attribute_model_1.Attribute.create({
                        name: row.Name,
                        unit: row.Unit,
                        slug,
            ...
            throw new ApiError_1.ApiError(500, "Failed to import attributes");
        }
    },
};
